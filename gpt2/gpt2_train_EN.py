...
def setup_train_args():
    ...
    parser.add_argument('--train_raw_path', default='data/train_data_translated.json', type=str, required=False, help='原始训练语料')
    parser.add_argument('--train_tokenized_path', default='data/train_medEN_tokenized.txt', type=str,
                        required=False,
                        help='将原始训练语料tokenize之后的数据的存放位置')
    parser.add_argument('--valid_raw_path', default='data/validate_data_translated.json', type=str, required=False, help='原始验证语料')
    parser.add_argument('--valid_tokenized_path', default='data/validate_medEN_tokenized.txt', type=str,
                        required=False,
                        help='将原始验证语料tokenize之后的数据的存放位置')
    parser.add_argument('--test_raw_path', default='data/test_data_translated.json', type=str, required=False, help='原始测试语料')
    parser.add_argument('--test_tokenized_path', default='data/test_medEN_tokenized.txt', type=str,
                        required=False,
                        help='将原始测试语料tokenize之后的数据的存放位置')
    parser.add_argument('--log_path', default='data/trainingEN.log', type=str, required=False, help='训练日志存放位置')
    parser.add_argument('--raw', default=True, action='store_true', help='是否对原始训练语料做tokenize。若尚未对原始训练语料进行tokenize，则指定该参数')
    parser.add_argument('--epochs', default=20, type=int, required=False, help='训练的轮次')
    ...
    parser.add_argument('--dialogue_model_output_path', default='dialogue_model/', type=str, required=False,
                        help='对话模型输出路径')
    parser.add_argument('--pretrained_model', default='dialogue_model/GPT2_pretrained_EN/', type=str, required=False, help='预训练的GPT2模型的路径')
    parser.add_argument('--pretrained_model_token', default='dialogue_model/GPT2_pretrained_EN/tokenizer.json', type=str, required=False,
                        help='预训练的GPT2模型的Tokenizer')
...
def preprocess_raw_data(args, tokenizer, n_ctx):
    ...
    with open(args.train_raw_path, 'rb') as json_file:
...
# Start Valid Set
    ...
    with open(args.valid_raw_path, 'rb') as json_file:
        # train_data = f.read().decode("utf-8")
...
# Start Test Set
    ...
    with open(args.test_raw_path, 'rb') as json_file:
        # train_data = f.read().decode("utf-8")
...
def evaluate(model, device, test_list, multi_gpu, args):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn, drop_last=True)
    total_num = 1
    ave_acc = 0
    ave_loss = 0
    ave_ppl = 0
...
#            tb_writer.add_scalar('loss', loss.item(), overall_step)
        print("evaluate overall: loss {} ,perplexity {} ,accuracy {}".format(ave_loss / total_num, ave_ppl / total_num, ave_acc / total_num))
...
#                                                                                accuracy))
        ...
        if args.train_mmi:  # 当前训练MMI模型
            ...
            model_path = join(args.dialogue_model_output_path, 'medEN_model_epoch{}'.format(epoch + 1))
...
def main():
    ...
    # tokenizer = BertTokenizer.from_pretrained(args.pretrained_model_token)
